...
|信道分组-选择带宽|下拉框||必选项|失去焦点和提交时为空，提示“不能为空”|选项包含：125kHz、250kHz、500kHz，选择对应的带宽后，下列表格显示出对应的信道分组。|
|信道分组|单选按钮||必选项|失去焦点和提交时为空，提示“不能为空”|1.选择带宽后，表格形式显示对应的信道分组，信道分组信息详见文档[FDD跳频逻辑设计文档]()；2.单选按钮选择其中一组信道即可。|
|备注信息|单行输入框||非必填项，100字符，中英文数字符号|||可填写修改信道的原因等信息。|
|提交|按钮|提交成功，页面返回信道配置页面。|||点击提交，成功提交立即调整指定基站的信道传输信息。|
|取消|按钮|页面返回信道配置页面|||取消操作，不执行修改。|

##### 3.7.1.2 信道修改管理
* 修改信道时为批量修改，下列表格以单个基站为单位记录修改信道的操作，字段包含：基站ID、带宽、信道组、修改人、修改时间以及备注信息。
* 可通过基站ID进行精准检索；
* 通过基站管理中点击基站ID进入详情，在信道信息处点击“信道配置”，页面跳转至“新增信道配置”页面并且基站ID的多行输入框中默认显示详情来源的基站ID。
...
|设置预警值|单行输入框||非必填项，数字、2字符||1.填写数字，表示监控事件的剩余量，例如：事件为监控电量，填写80%，表示当设备电量剩余80%的时候触发预警通知；2.是否可以添加预警值详见下方表格。|
...
* 紧急：
1、在无应答的情况下每个联系人电话拨打两次，两次间隔5分钟；
2、优先联络人电话应答，则无需继续拨打；
3、无论应答与否，均需短信给已拨打电话的联络人报警事件，模版：报警事件ID：1212，事件类型：XXXXXX；
